...
class OutputManager:
    ...
    async def _materialize_images(self, markdown_content: str, base_dir: Path, prefix: str) -> tuple[str, int]:
        ...
        url_img_re = re.compile(r"!\[(?P<alt>.*?)\]\((?P<url>https?://[^\s)]+)\)")
        ...
        if url_matches:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                for m, result in zip(url_matches, results, strict=False):
    ...
    async def save_batch_results(self, results: list[str], filenames: list[str], source_files: list[Path] = None, include_page_headlines: bool = False) -> list[tuple[Path, int]]:
        """Save multiple OCR results to files."""
        output_files = []
        for i, (content, filename) in enumerate(zip(results, filenames, strict=False)):
